...
//we can consider every value from n - 1 to 0. Fix the amount of occurrences of that value, and compute
//per configuration, what's the total score from all the values greater. 

//this can be done in linear time using some weird dp. 
...
signed main() {
    ...
    while(t--){
        ...
        vm p(1, 0), w(1, 1);
        for(int i = n - 1; i >= 0; i--){
            vm pfx(p.size() + 1, 0), sfx(p.size(), 0);
            for(int j = 1; j <= p.size(); j++){
                pfx[j] = pfx[j - 1] + p[j - 1] * w[j - 1];
            }   
            sfx[p.size() - 1] = w[p.size() - 1];
            for(int j = (int) w.size() - 2; j >= 0; j--){
                sfx[j] = sfx[j + 1] + w[j];
            }
            vm np(f[i] + 1, 0), nw(f[i] + 1, 0);
