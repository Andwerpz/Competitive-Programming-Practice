...
class revegetatever1 {
	...
	//probably in the group calculation section; it doesn't calculate the groups right
    ...
    for(int i = 0; i < groups.size(); i++) {
    	...
    	if(pasture[groups.get(i).get(0)] == 0) {
    	...
    	while(toProcess.size() != 0 && isBipartite) {
    		...
    		if(cur == 7986) {
    			...
    			System.out.println("7986 detected");
    			
    			System.out.println(pasture[7986]);
    			
    		}
    		
    		if(cur == 8923) {
    			
    			System.out.println("8923 detected");
    			
    			System.out.println(pasture[8923]);
    		...
    		for(int j = 0; j < connections.get(cur).size(); j++) {
    			...
    			if(!visited.contains(connections.get(cur).get(j)[0])) {
    				...
    				if(connectionType == 0) {
    					...
    					if(curNodeColor == 1) {
    						System.out.println("setting node " + connections.get(cur).get(j)[0] + " to " + 2);
    						pasture[connections.get(cur).get(j)[0]] = 2;
    					}
    					
    					else {
    						System.out.println("setting node " + connections.get(cur).get(j)[0] + " to " + 1);
