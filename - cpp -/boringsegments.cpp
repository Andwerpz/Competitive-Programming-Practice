...
//first, sort the segments in ascending order according to weight. Then, we can do a 2 pointer solution and for every
//l, find the minimum r such that if we only pick segments with weight in range [l, r], the set is good. 

//to quickly check if a set is good, we can use a lazy segment tree with range increment and range minimum query. 

//minor implementation detail: since we not only need to cover every point, but the segments need to overlap, we can 
//seperate every adjacent point by another auxillary point to make sure that the segments are overlapping.
...
signed main() {
    ...
    int r = 0;
    for(int i = 0; i < n; i++){
        while(r != n && segt.query(0, m) == 0){
            segt.modify(a[r].second.first, a[r].second.second, 1);
            r ++;
        }
        if(segt.query(0, m) != 0){
            ans = min(ans, a[r - 1].first - a[i].first);
        }
        segt.modify(a[i].second.first, a[i].second.second, -1);
    }
    cout << ans << "\n";
