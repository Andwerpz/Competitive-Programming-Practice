...
//2023-2024 ICPC Asia Jakarta Regional F
...
signed main() {
    ...
    int n, m;
    cin >> n >> m;
    vector<pair<ll, pair<ll, ll>>> event(0);    //{index, {time, val}}
    for(int i = 0; i < m; i++){
        ll l, r, x;
        cin >> l >> r >> x;
        l --;
        event.push_back({l, {i, x}});
        event.push_back({r, {i, -x}});
    }
    ...
    Segtree<seg> segt(m, {0, 0, 0, 0}, {0, 0, 0, 0}, fmodify, fcombine);
    int q;
    cin >> q;
    vector<vector<int>> queries(q, vector<int>(4, 0));  //{index, start time, end time, qindex}
    for(int i = 0; i < q; i++){
    ...
    for(int i = 0; i < q; i++){
        ...
        int t_start = queries[i][1];
        int t_end = queries[i][2];
        int q_ind = queries[i][3];
        while(ptr != event.size() && event[ptr].first <= index) {
            int time = event[ptr].second.first;
            ll val = event[ptr].second.second;
            ptr ++;
            seg n_val(val);
            segt.modify(time, n_val);
        }
        ans[q_ind] = (segt.query(t_start, t_end).max_sum);
    }
    for(int i = 0; i < q; i++){
        cout << ans[i] << "\n";
    }
