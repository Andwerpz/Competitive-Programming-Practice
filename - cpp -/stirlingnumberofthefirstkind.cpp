...
//judge.yosupo.jp - stirling_number_of_the_first_kind

//computes all stirling numbers of the first kind for fixed k in O(n * log(n)) time. 
//currently calculates for arbitrary mod, can do better if we use NTT with the specific mod provided.
...
void fac_init(int N) {
    ...
    for(int i = N; i > 0; i--) invfac[i - 1] = invfac[i] * mint(i);
}
