...
mint calc_1(int n){
    mint ans = 0;
    for(int k = 0; k <= n; k++){
        ans += nck(n + k, k).inv_divide(mint(2).pow(k));
    }
    return ans;
}
...
signed main() {
    ...
    for(int i = 1; i < 20; i++){
        // mint cur = f1[i - 1];
        // cur += nck(2 * i - 1, i).inv_divide(mint(2).pow(i));
        // cur += (f1[i] - nck(2 * i, i).inv_divide(mint(2).pow(i))).inv_divide(2);
        mint cur = f1[i - 1] * 2;
        cur += nck(2 * i - 1, i).inv_divide(mint(2).pow(i - 1));
        cur -= nck(2 * i, i).inv_divide(mint(2).pow(i));
        cout << cur << "\n";
        cout << nck(2 * i - 1, i) << " " << nck(2 * i, i) << "\n";
    }
