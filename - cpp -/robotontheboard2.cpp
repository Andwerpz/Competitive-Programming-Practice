
#include <iostream>
#include <fstream>
#include <sstream>
#include <vector>
#include <string>
#include <bits/stdc++.h>

using namespace std;

typedef long long ll;
typedef pair<int, int> pii;

//Codeforces - 1607F

//came up with the right answer, my implementation just sucks. 
//in cpp, instead of vector<int>, use pair<int, int>

//you just have to check the size of path for each cell in the grid. Notice that from each cell, since each cell has only 1 outgoing edge,
//there is only 1 possible path for the robot to take. This means that if you process a path, and another path happens to intersect
//with that already processed path, you don't need to process the rest of the path, since you already know where it is going. 

int main() {
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);

    int t = 0;
    cin >> t;
    while(t-- > 0){
        int n = 0;
        int m = 0;
        cin >> n >> m;
        vector<string> board = vector<string>(n);
        for(int i = 0; i < n; i++){
            string s;
            cin >> s;
            board[i] = s;
        }
        vector<vector<int>> steps = vector<vector<int>>(n);
        for(int i = 0; i < n; i++){
            steps[i] = vector<int>(m);
            for(int j = 0; j < m; j++){
                steps[i][j] = -1;
            }
        }
        for(int i = 0; i < n; i++){
            for(int j = 0; j < m; j++){
                if(steps[i][j] == -1){
                    bool loop = false;
                    set<pii> visited = set<pii>();
                    stack<pii> order = stack<pii>();
                    visited.insert({i, j});
                    order.push({i, j});
                    int x = i;
                    int y = j;
                    while(true){
                        char nextMove = board[x][y];
                        if(nextMove == 'L') {
                            y --;
                        }
                        else if(nextMove == 'R') {
                            y ++;
                        }
                        else if(nextMove == 'U') {
                            x --;
                        }
                        else {
                            x ++;
                        }
                        //first check if we've hit a loop, or out of bounds
                        if(x < 0 || y < 0 || x >= n || y >= m) {
                            break;
                        }
                        pii next = {x, y};
                        order.push(next);
                        if(visited.find(next) != visited.end()) {
                            loop = true;
                            break;
                        }
                        //we aren't in loop, or out of bounds. Check if we've already visited this cell
                        if(steps[x][y] != -1) {
                            break;
                        }
                        //this is our first time visiting this cell. 
                        visited.insert(next);
                    }
                    if(loop) {	
                        //we need to first assign the same value to all cells that are in the loop
                        pii loopEnd = order.top();
                        order.pop();
                        vector<pii> loopCells = vector<pii>();
                        while(true) {
                            pii next = order.top();
                            order.pop();
                            loopCells.push_back(next);
                            if(loopEnd.first == next.first && loopEnd.second == next.second) {
                                break;
                            }
                        }
                        for(pii a : loopCells) {
                            steps[a.first][a.second] = loopCells.size() - 1;
                        }
                        //now just proceed like normal
                        int counter = loopCells.size();
                        while(order.size() != 0) {
                            pii next = order.top();
                            order.pop();
                            steps[next.first][next.second] = counter;
                            counter ++;
                        }
                    }
                    else {
                        pii last = order.top();
                        order.pop();
                        int counter = 0;
                        if(steps[last.first][last.second] == -1) {	//new cell. Initialize value to 0
                            steps[last.first][last.second] = 0;
                            counter = 1;
                        }
                        else {	//already visited cell. Start with value in cell
                            counter = steps[last.first][last.second];
                            counter ++;
                        }
                        while(order.size() != 0) {
                            pii next = order.top();
                            order.pop();
                            steps[next.first][next.second] = counter;
                            counter ++;
                        }
                    }
                }
            }
        }
        int r = 0;
        int c = 0;
        int d = steps[0][0];
        for(int i = 0; i < n; i++) {
            for(int j = 0; j < m; j++) {
                if(steps[i][j] > d) {
                    r = i;
                    c = j;
                    d = steps[i][j];
                }
            }
        }
        r ++;
        c ++;
        d ++;
        cout << r << " " << c << " " << d << endl;
    }

    return 0;
}



