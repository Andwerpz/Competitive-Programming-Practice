...
//Note that each mountain is equivalent to a segment on the ground, and that the total distance is just the length
//of the union of all segments * sqrt(2). 

//to keep track of the length of union of segments, we can use a lazy segment tree that allows us to query for the count
//of smallest elements over a range. We first need to coordinate compress all the queries tho. 
