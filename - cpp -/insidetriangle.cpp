...
//2025 ICPC Asia Chengdu Regional Contest - I
...
//computes the signed angle ABC
ld pt3_angle(vec2 a, vec2 b, vec2 c) {
    vec2 u = a - b;
    vec2 v = c - b;
    return asin(cross(u, v) / (u.length() * v.length()));
}
...
signed main() {
    ...
    while(t--) {
        ...
        vec2 q_center = polygon_centroid(q);
        int mxptr = 0, mnptr = 0;
        ld mxang = -1e9, mnang = 1e9;
        {
            for(int i = 0; i < m; i++) {
                ld ang = pt3_angle(q_center, p[0], q[i]);
                if(ang > mxang) {
                    mxptr = i;
                    mxang = ang;
                }
        ...
        ll ans = 0;
        for(int i = 0; i < n; i++) {
            mxang = pt3_angle(q_center, p[i], q[mxptr]);
            mnang = pt3_angle(q_center, p[i], q[mnptr]);
            while(true) {
                ld nmxang = pt3_angle(q_center, p[i], q[(mxptr + 1) % m]);
                if(nmxang > mxang) {
                    mxang = nmxang;
                    mxptr = (mxptr + 1) % m;
                }
            ...
            while(true) {
                ld nmnang = pt3_angle(q_center, p[i], q[(mnptr + 1) % m]);
                if(nmnang > mnang) {
                    mnang = nmnang;
                    mnptr = (mnptr + 1) % m;
                }
