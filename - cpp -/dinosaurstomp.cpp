...
//UTPC Contest 10-15-25 Div.1 - E

//bruh, my segtree broke. 

//you can try every possible move for cole, and then find the maximum subarray sum for barney to stomp. 
//this can be done in log(n) time using a lazy segtree. 
...
signed main() {
    ...
    SegtreeLazy<ll> segt(n - k + 1, 0, (ll) -1e9, 0, fmodify, fmodifyk, fcombine);
    vl a(n), pfx(n + 1, 0);
    for(int i = 0; i < n; i++) {
        cin >> a[i];
    }
    ...
    for(int i = 0; i < n; i++) {
        ...
        int r = min(n - k + 1, i + 1);
        segt.modify(l, r, -a[i]);
        ans = min(ans, segt.query(0, n - k + 1));
        segt.modify(l, r, a[i]); 
    }
    cout << ans << "\n";
