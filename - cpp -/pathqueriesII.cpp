...
//finally works :))
//was originally querying in O(n * log(n)^2), but managed to get rid of a log factor by caching results
//to maximum heavy segments. 
...
int main() {
    ...
    HLD<int> hld(n, 0, c, 0, -1e9, 0, fmodify, fmodifyk, fcombine);
    for(int i = 0; i < n; i++){
        hld.modify(i, val[i]);
    }
    for(int i = 0; i < q; i++){
