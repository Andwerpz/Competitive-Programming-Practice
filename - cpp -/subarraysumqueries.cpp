...
//each segment stores maximum prefix and suffix sums, the total sum, and the maximum subarray sum over
//itself. 

//we can then combine two adjacent segments in O(1) time, and modifying a single element only takes
//O(log(n)) time. 
