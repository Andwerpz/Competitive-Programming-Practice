...
public class additiontosegmentver1 {
	...
	public static void setValueInRange(int low, int high, int value, int priority) {	//low inclusive, high exclusive, replacing the old value with new value
		for(int i = low; i < high; i += 0) {
			...
			tree.get(layer).get(index).setValue(value, priority);
		}
	}
...
class Node {
	...
	public void setValue(int value, int priority) {
		this.value = value;
		this.priority = priority;
	}
