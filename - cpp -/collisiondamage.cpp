...
//2025 ICPC Shenyang Regional - G

//template check problem lmao

//given two convex polygons A, B, we want to compute the expected intersection area
//given that A is uniformly randomly translated in such a way that it intersects B.

//consider the probability of some point on B, b', being covered by A. If we fix some point on A 
//to be its anchor, a', then we notice that the area of the region that a' can be in such that
//b' is covered by A is actually equal to A. And this is true regardless of what point b' we pick.
//so, this means that all points on B have a uniform probability for A to cover them. 

//so to compute this probability, we need to figure out the total area that a' can be in. This is just
//the area of the minkowski sum of A and B. Then, we just divide the area of A by the area of the minkowski
//sum, and then multiply by the area of B as we're integrating over it to get the EV. 
...
signed main() {
    ...
    while(t--) {
        ...
        vector<vec2> c;
        for(int i = 0; i < n; i++) for(int j = 0; j < m; j++) c.push_back(a[i] - b[j]);
        c = convex_hull(c);
        ld ans = polygon_area(a) * polygon_area(b) / polygon_area(c);
